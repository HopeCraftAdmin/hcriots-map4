package net.hcriots.hcf.armors.bard;
...
import com.hcriots.utils.other.chat.Lang;
				...
				if (held != null) {
					...
					if (!plugin.getFactionManager().getFactionAt(player.getLocation()).isSafezone()) {
						...
						if (playerFaction != null) {
							...
							nearbyEntities.stream().filter(nearby -> nearby instanceof Player && !player.equals(nearby)).forEach(nearby -> {
	...
	public void onPlayerInteract(final PlayerInteractEvent event) {
		...
		if (action == Action.RIGHT_CLICK_AIR || (!event.isCancelled() && action == Action.RIGHT_CLICK_BLOCK)) {
			...
			if (bardData != null) {
				...
				player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&eYou have just used a &aBard Buff &ethat cost you &lsome &eof your Energy."));
