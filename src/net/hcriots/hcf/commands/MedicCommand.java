...
public class MedicCommand implements CommandExecutor
{
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        if (Cooldowns.isOnCooldown("medic_cooldown", p)) {
            p.sendMessage(ChatColor.RED + "You still have a " + ChatColor.DARK_GRAY + ChatColor.BOLD + "Amethyst" + ChatColor.RED + " cooldown for another " + (Cooldowns.getCooldownForPlayerLong("medic_cooldown", p)) + ChatColor.RED + '.');
            return true;
        }
        ...
        if (sender instanceof Player) {
            ...
            Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&',"&8&l" + p.getName().toUpperCase() + " &7has used their Amethyst Rank &eto revive &8&l" + target.getName().toUpperCase() + "&7."));
        }
