...
public class RenameCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    	...
    	if(args.length < 1) {
			...
			p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&7If you want to use any color make sure you do & and then the color code."));
		} else {
			...
			if(hand.getType() != Material.AIR) {
				...
				p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&aYou have successfully renamed your &c" + hand.getType().name().toLowerCase() + " &7to &r" + name));
			} else {
